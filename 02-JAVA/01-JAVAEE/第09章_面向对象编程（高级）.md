...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121389.png" alt="第2阶段：Java面向对象编程-第08章" style="zoom: 40%;" />
...
![image-20220325213629311](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121390.png)
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121391.png" alt="image-20220514183814514" style="zoom:67%;" />
...
![image-20220104100145059](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121392.png)
...
运行不会报错，但是应该直接从类访问静态变量，并非从实例进行访问
```
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121393.png" alt="image-20220520174508815" style="zoom: 80%;" />
...
如果我们要让类在一个虚拟机中只能产生一个对象，我们首先必须将类的构造器的访问权限设置为`private`，这样，就不能用`new`操作符在类的外部产生类的对象了，但在类内部仍可以产生该类的对象。因为在类的外部开始还无法得到类的`对象`，只能调用该类的某个`静态方法`以返回类内部创建的对象，`静态方法`只能访问类中的静态成员变量，所以，指向类内部产生的该类对象的变量也必须定义成静态的。
### 2.4 单例模式的两种实现方式
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121394.png" alt="image-20220325222541203" style="zoom:67%;" />
...
![image-20220325223215924](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121395.png)

IDEA工具：

（1）配置运行参数

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121396.png" alt="image-20211228101828718" style="zoom: 80%;" />

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121397.png" alt="image-20211228102022216" style="zoom:67%;" />

（2）运行程序

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121398.png" alt="image-20211228102059327" style="zoom: 80%;" />
...
在Java中，类的文件名应该与类名一致，并且每个Java文件只能包含一个公共类。虽然在这种情况下类的文件名不同于类名，但是根据你给出的代码，这段代码可以正常编译和运行。

命令行编译：

bash
javac OtherThing.java
命令行运行：

bash
java Something
这段代码将会输出 "Do something ..." 到控制台。
    
在Java中，public static void main(String[] args)是作为程序的入口点的约定，这个方法签名是固定的。在你提供的代码中，虽然参数名并不是传统的args，但是这并不会导致编译错误。

在Java中，方法的参数名并不影响方法的签名，只要参数的类型和顺序是正确的，就可以通过编译。因此，使用String[] something_to_do作为 main 方法的参数，虽然不符合通常的命名惯例，但仍然是合法的，不会导致编译错误。
    
    Java中，类的文件名通常应该与类名相匹配，这是为了方便代码的组织和维护。然而，这并不是强制性的要求。

当你编译一个Java程序时，编译器会根据公共类的类名来确定生成的.class文件的名称。因此，尽管文件名与类名不一致，编译器仍然能够正确地生成对应的.class文件。

在运行Java程序时，虚拟机会根据类名来加载类。由于编译生成的.class文件包含了类的定义，而类的定义中包含了类的名字，因此虚拟机可以根据类的定义来正确加载程序，而不会受到文件名的影响。
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121399.png" alt="image-20220325230208941" style="zoom:67%;" />
...
![image-20220325231608838](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121400.png)
...
![image-20220517204707255](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121401.png)
...
![image-20220325232847872](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121402.png)
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121403.png" alt="image-20220503145003315" style="zoom: 80%;" />

制作月饼的模板：

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121405.png" alt="image-20220517205013997" style="zoom: 50%;" />
...
![image-20220504164925878](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121406.png)
...
![bbcc80f541000c71b81650cfaa770c86](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121407.png)
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121408.png" alt="image-20220517211517846" style="zoom:67%;" />

- 例如：Java程序是否能够连接使用某种数据库产品，那么要看该数据库产品能否实现Java设计的JDBC规范

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121409.png" alt="image-20220325235434103" style="zoom:67%;" />
...
public interface USB3{
    //静态常量，接口中的变量默认是公共（public）和静态（static）的，所以可以直接使用接口名访问该常量
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121410.png" alt="image-20220514163212312" style="zoom:80%;" />
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121411.png" alt="image-20220514163311418" style="zoom:80%;" />

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121412.png" alt="image-20220325235321778" style="zoom:67%;" />

![1562216188519](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121413.png)
...
![](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121414.jpg)
...
![image-20220328002053452](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121415.png)
...
![image-20220504172547709](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121416.png)

其中，有一个乘坐交通工具的方法takingVehicle()，在此方法中调用交通工具的run()。为了出行方便，他买了一辆捷安特自行车、一辆雅迪电动车和一辆奔驰轿车。这里涉及到的相关类及接口关系如下：

![image-20220504172918861](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121417.png)
...
![image-20221124223912529](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121418.png)
...
Annotation 可以像修饰符一样被使用，可用于修饰`包、类、构造器、方法、成员变量、参数、局部变量`的声明。还可以添加一些参数值，这些信息被保存在 Annotation 的 “name=value” 对中。
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121419.png" alt="image-20220511181800694" style="zoom: 50%;" />

![image-20220524102038600](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121420.png)
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121421.png" alt="image-20211228180938922" style="zoom:80%;" />

![image-20221002195547325](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121422.png)

其中，junit-libs包内容如下：

![image-20220813005206452](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121423.png)

第2步：选择要在哪些module中应用JUnit库

![image-20220813005511062](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121424.png)

第3步：检查是否应用成功

![image-20220813005729233](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121425.png)

**注意Scope：选择Compile，否则编译时，无法使用JUnit。**

第4步：下次如果有新的模块要使用该libs库，这样操作即可

![image-20220813005944022](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121426.png)

![image-20220813010018152](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121427.png)

![image-20220813010055217](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121428.png)

![image-20220813010124381](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121429.png)
...
![image-20220106152412245](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121430.png)
...
![image-20220813011625546](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121431.png)

![image-20220813011642180](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121432.png)
...
![image-20211229100040505](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121433.png)
...
1. 为基本值提供分类功能。这些功能大多数于各种转换有关：在基本值和String对象间相互转换，在基本值和String对象之间按不同基数转换，如二进制、八进制和十六进制

### 11.2 有哪些包装类
包装类型可以为 null，而基本类型不可以，别小看这一点区别，它使得包装类型可以应用于 POJO 中，而基本类型则不行。

POJO 是什么呢？这里稍微说明一下。

POJO 的英文全称是 `Plain Ordinary Java Object`，翻译一下就是，简单无规则的 Java 对象，只有属性字段以及 setter 和 getter 方法，示例如下。
和 POJO 类似的，还有数据传输对象 DTO（Data Transfer Object，泛指用于展示层与服务层之间的数据传输对象）、视图对象 VO（View Object，把某个页面的数据封装起来）、持久化对象 PO（Persistant Object，可以看成是与[数据库](https://cloud.tencent.com/solution/database?from_column=20065&from=20065)中的表映射的 Java 对象）

Java针对八种基本数据类型定义了相应的引用类型：包装类（封装类）。有了类的特点，就可以调用类中的方法，Java才是真正的面向对象。

<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121434.png" alt="image-20220329001912486" style="zoom:80%;" />
...
<img src="https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121435.png" alt="image-20220514163725830" style="zoom:80%;" />
...
![image-20220813012801907](https://raw.githubusercontent.com/sameal153/PicturePool/master/img/202311082121436.png)
